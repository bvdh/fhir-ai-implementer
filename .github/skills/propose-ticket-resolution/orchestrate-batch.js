 ...
 */
function findBatchTickets() {
  const fromCurrentTickets = loadCurrentTickets();
  if (fromCurrentTickets.length > 0) {
    return fromCurrentTickets.filter((ticketKey) => {
      const ticketDir = findTicketDir(ticketKey);
      return Boolean(ticketDir);
    });
  }
  ...
  return Array.from(unresolvedTickets).sort((a, b) => parseInt(a.split('-')[1], 10) - parseInt(b.split('-')[1], 10));
}
...
async function main() {
  ...
  if (args[0] === '--all') {
    ...
    // Find batch tickets
    console.log('\nCollecting tickets for batch...');
    const unresolvedTickets = findBatchTickets();
    ...
    console.log('Summary');
    console.log('='.repeat(70));
    console.log(`Total tickets:                ${unresolvedTickets.length}`);
    ...
    if (fs.existsSync(CURRENT_TICKETS_FILE)) {
      console.log(`\nScope source: currentTickets.md (${path.relative(REPO_ROOT, CURRENT_TICKETS_FILE)})`);
    } else {
      console.log('\nScope source: unresolved ticket scan fallback (currentTickets.md not found)');
    }
    console.log('✓ Orchestration plan saved to: jira/orchestration-plan.json');
