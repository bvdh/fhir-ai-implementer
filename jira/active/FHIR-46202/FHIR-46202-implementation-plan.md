...
|---|---|---|---|---|---|
| FHIR-46202 | Correct regex in the documentation for code type | Resolved - change required | fhir-fork/source/datatypes.html (code datatype section, datatypes.html#code) | Technical correction (regex text/value) | Requested regex from ticket: ^[^\s]+(\s[^\s]+)*$ |
...
- [ ] Regex for code resolves to exactly ^[^\s]+(\s[^\s]+)*$ in rendered documentation
...
- Assumption: The ticket-provided regex (^\[^\s\]+(\s\[^\s\]+)*$ when unescaped) is authoritative for the correction.
