# FHIR-47470: Bundle.link - add preferred ValueSet for most common values

## Metadata

- **__EMPTY**: Lloyd McKenzie
- **__EMPTY_1**: Public
- **__EMPTY_2**: We're in favor of doing this, but are unsure:

a) whether the tooling for the core spec supports publishing additional bindings

b) if not, how hard it would be to make this happen.

If it's too hard, we could just include a comment listing the most common/typical codes.
- **Assignee**: Unassigned
- **Comments**: 9/16/24
- **Created**: 9/12/24
- **Description**: Bundle.link.relation points to alueset-iana-link-relations which has 120 entries.

The navigation links (first, last, self, prev, next) are used in the vast majority of the cases. Add a preferred ValueSet to binding.additional that indicates these as preferred.
- **Grouping**: [Ready-For-Vote]
- **Issue Type**: Change Request
- **Key**: FHIR-47470
- **Related Artifact(s)**: Bundle
- **Reporter**: Bas van den Heuvel
- **Resolution**: Persuasive with Modification
- **Resolution Description**: Will create a 'starter' additional binding as proposed with one additional code: [stylesheet|https://build.fhir.org/codesystem-iana-link-relations.html#iana-link-relations-stylesheet] which is used in documents.
- **Status**: Resolved - change required
- **Summary**: Bundle.link - add preferred ValueSet for most common values
- **Work Group**: FHIR Infrastructure

- **Jira**: https://jira.hl7.org/browse/FHIR-47470

## Comments

### 1. Lloyd McKenzie - 9/16/24
Security: Public

We're in favor of doing this, but are unsure:

a) whether the tooling for the core spec supports publishing additional bindings

b) if not, how hard it would be to make this happen.

If it's too hard, we could just include a comment listing the most common/typical codes.

### 2. Lloyd McKenzie - 1/5/26
Security: Public

We're doing this for Patient.language, so assume it's ok

